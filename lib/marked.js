...
Lexer.prototype.lex = function(src) {
  ...
  return this.token(src, true, false, this.options.lineNumberOffset || 1);
};
...
Lexer.prototype.token = function(src, top, bq, line) {
  ...
  while (src) {
    ...
    if (cap = this.rules.blockquote.exec(src)) {
      ...
      this.token(cap, top, true, blockquoteLine);
    ...
    if (cap = this.rules.list.exec(src)) {
      ...
      for (; i < l; i++) {
        ...
        // Recurse.
        this.token(item, false, bq, listLine);
        listLine += cap[i].split('\n').length - 1;
